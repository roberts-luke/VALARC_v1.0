...
# calculate MXL in Eurasian Basin using density criteria (0.03 kg/m3 difference between rho at surface and MLD)
FILEOUT=nemo_${RUN_NAME}o_${FREQ}_${TAG}_grid-${GRID}_mxl.nc
...
# averaging MXL depth in Eurasian Basin: 60° W–50° W, 55° N–62° N
ijbox=$($CDFPATH/cdffindij -c mesh.nc -w 10.000 10.000  84.000  88.000 | tail -2 | head -1)
echo ijbox
$CDFPATH/cdfmean -f $FILEOUT -v 'somxl030' -p T -w ${ijbox} 0 0 -minmax -o tmp_EB_MXL_$FILEOUT
#-minmax computes & outputs the min and max in .nc file

# mv output file
if [[ $? -eq 0 ]]; then
   mv tmp_EB_MXL_$FILEOUT EB_MXL_$FILEOUT
else
